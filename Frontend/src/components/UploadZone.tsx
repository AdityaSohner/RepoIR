  ...
  const pollJob = (jobId: string, name: string) => {
    ...
    const id = setInterval(async () => {
      ...
      try {
        ...
        if (res.status === 'completed' || res.status === 'failed' || res.status === 'skipped') {
          clearInterval(id);
          if (res.status === 'completed') {
            ...
            addNotification(`"${name}" skipped: ${res.error || 'No speech detected'}`, 'warning');
          } else {
        ...
        {activeTab === 'file' && (
          ...
          >
            ...
            <p className="text-sm text-muted-foreground">
              Supports images, PDFs, DOCX, PPTX, XLSX, audio, and text files
            </p>
            {!vaultPassword && (

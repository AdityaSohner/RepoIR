...
export default function FilePreviewModal({
  ...
  const isImage = file ? (IMAGE_TYPES.includes(fileType) || file.type === 'image') : false;
  const isPDF = fileType === '.pdf';
  const isOffice = OFFICE_TYPES.includes(fileType);
  const isText = file?.type === 'text';
  const isUrl = file?.type === 'url';
  const isAudio = file ? (file.type === 'audio' || ['.mp3', '.wav', '.ogg', '.m4a', '.flac', '.webm'].includes(fileType)) : false;
  const hasGDrivePath = Boolean(file?.file_path);

  const previewUrl = file ? getPreviewUrl(file.object_id, vaultPassword) : '';
  const driveEmbedUrl = hasGDrivePath && file
  ...
  const renderPreviewContent = () => {
    ...
    if (isAudio) {
      return (
        <div className="flex flex-col items-center justify-center min-h-[50vh] bg-muted/20 rounded-2xl border border-border p-8 text-center animate-in fade-in zoom-in duration-300">
          <div className="w-20 h-20 rounded-2xl bg-primary/10 flex items-center justify-center mb-6">
            <HiOutlineVolumeUp className="w-10 h-10 text-primary" />
          </div>
          <h3 className="text-xl font-bold text-foreground mb-2 truncate max-w-lg">{file.source}</h3>
          <p className="text-sm text-muted-foreground mb-8">
            Audio Track ({fileType.toUpperCase().replace('.', '')})
          </p>
          
          <div className="w-full max-w-xl bg-card rounded-2xl border border-border p-4 shadow-lg mb-6">
            <audio
              controls
              src={previewUrl}
              className="w-full"
          ...
          {hasGDrivePath && (
            <button
              onClick={openInDrive}
              className="px-6 py-2.5 rounded-xl border border-border hover:bg-muted font-medium transition-all flex items-center gap-2 text-sm text-muted-foreground"
            >
              ...
              {file.icon_url ? (
                ...
                <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-primary/10 flex-shrink-0">
                  {isImage ? (
                    ...
                    <HiOutlineVolumeUp className="w-5 h-5 text-primary" />
                  ) : (
